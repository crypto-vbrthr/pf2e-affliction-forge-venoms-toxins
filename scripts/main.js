import {
  VENOMS_TOXINS_CONTENT_VERSION,
  VENOMS_TOXINS_DEFINITIONS,
  VENOMS_TOXINS_MODULE_ID
} from "./content.js";

const MODULE_ID = VENOMS_TOXINS_MODULE_ID;
const CORE_MODULE_ID = "pf2e-affliction-forge";
const PROVIDER_ID = "venoms-toxins";
const LIBRARY_ID = "venoms-toxins.core";
const PACK_NAME = "affliction-forge-venoms-toxins";
...
async function createManagedPack() {
  ...
  try {
    return await CompendiumCollection.createCompendium({
      name: PACK_NAME,
      label: localize("PF2E_AFFLICTION_VT.Module.Library", "Venoms & Toxins"),
      type: "Item",
      package: "world",
      packageName: "world",
      system: "pf2e"
...
function addonFlags() {
  return {
    managed: true,
    contentVersion: VENOMS_TOXINS_CONTENT_VERSION,
...
function registerProvider(api) {
  ...
  api.providers.register({
    id: PROVIDER_ID,
    label: localize("PF2E_AFFLICTION_VT.Module.Title", "PF2E Affliction Forge: Venoms & Toxins"),
    moduleId: MODULE_ID,
    version: VENOMS_TOXINS_CONTENT_VERSION,
    metadata: {
      themes: ["poison", "venom", "toxin"],
    ...
    libraries: [{
      ...
      description: localize("PF2E_AFFLICTION_VT.Module.LibraryDescription", "Original venoms and toxins with semantic Creature Forge tags."),
      metadata: {
        themes: ["poison", "venom", "toxin"],
        semanticTagContract: "1.0.0",
        creatureForgeReady: true,
...
async function initialize(api) {
  ...
  console.info(`${MODULE_ID} | Content synchronized`, {
    ...result,
    total: VENOMS_TOXINS_DEFINITIONS.length,
    collection: PACK_COLLECTION,
    version: VENOMS_TOXINS_CONTENT_VERSION
  });
}
...
Hooks.once("pf2eAfflictionForgeReady", (api) => {
  initialize(api).catch((error) => {
    ...
    globalThis.ui?.notifications?.error?.(
      localize("PF2E_AFFLICTION_VT.Module.SyncFailed", "Venoms & Toxins could not synchronize its library. See console.")
    );
  });
});
